...
//Output: location 
//Agent Output: 
__FLAME_GPU_FUNC__ int outputdata(xmachine_memory_FluidParticle* xmemory, xmachine_message_location_list* location_messages) 
{
	add_location_message(location_messages, xmemory->id, xmemory->x, xmemory->y, xmemory->z, xmemory->vx, xmemory->vy, xmemory->vz);
...
//Output: 
//Agent Output: 
__FLAME_GPU_FUNC__ int inputdata(xmachine_memory_FluidParticle* xmemory, xmachine_message_location_list* location_messages) 
{
	//Agent position vector
	glm::vec3 agent_position = glm::vec3(xmemory->x, xmemory->y, xmemory->z);
	glm::vec3 agent_velocity = glm::vec3(xmemory->vx, xmemory->vy, xmemory->vz);

	//FluidParticles perceived centre
	glm::vec3 perceived_centre = glm::vec3(0.0f, 0.0f, 0.0f);
	int perceived_count = 0;

	//FluidParticles global velocity matching
	glm::vec3 global_velocity = glm::vec3(0.0f, 0.0f, 0.0f);

	//FluidParticles short range avoidance centre
    ...
    while(location_message)
	{
		...
		if (location_message->id != xmemory->id){
			...
			if (separation < (INTERACTION_RADIUS)){
				...
				glm::vec3 message_velocity = glm::vec3(location_message->vx, location_message->vy, location_message->vz);
	...
	xmemory->vx = agent_velocity.x;
	xmemory->vy = agent_velocity.y;
	xmemory->vz = agent_velocity.z;
